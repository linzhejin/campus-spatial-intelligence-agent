...
XX_WORDS = ['星湖','信息学部','信部','武测','珞瑜门','珞瑜二门','广埠屯','文澜门']
GX_WORDS = ['工学部','工部','武水','水院','洪波门','茶港门']
...
def is_gate(name):
    return bool(re.search(r'(珞珈门|凌波门|洪波门|珞瑜门|珞瑜二门|科技门|南三门|南二门|南门|北门|东门|西门|正门|牌坊|附中门|文澜门|茶港门|星湖门|枫园门|梅园门|桂园门|樱园门)$', name)) \
        or name.endswith('校门')
...
SUPPLEMENT_POIS = [
    ...
    {'name': '武汉大学文澜门', 'lng': 114.360450, 'lat': 30.545254,
     'address': '信息学部文澜门（广埠屯，高德核实）',
     'aliases': ['文澜门', '信息学部文澜门', '武大文澜门']},
    {'name': '武汉大学茶港门', 'lng': 114.354475, 'lat': 30.539325,
     'address': '工学部茶港门（茶港路与天鹅路交叉口，高德核实）',
     'aliases': ['茶港门', '工学部茶港门', '武大茶港门']},
]
_CN = '零一二三四五六七八九'
...
def main():
    ...
    # 例外：校门（is_gate）位于校园边界，多边形易漏，强制保留
    before = len(kept)
    kept = [k for k in kept if in_campus(k['lng'], k['lat']) or is_gate(k['name'])]
